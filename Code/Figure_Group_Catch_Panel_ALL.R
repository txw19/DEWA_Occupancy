...
for(i in 1:out$BUGSoutput$n.sim){ # simulations
	for(t in 1:T){ #species
		for(n in 1:length(new.catch.stdz)){ # species
			probs[i,t,n] <- plogis(out$BUGSoutput$sims.list$alpha.group[i,t] + out$BUGSoutput$sims.list$beta.group7[i,t]*mean(loglwd) + out$BUGSoutput$sims.list$beta.group3[i,t] * new.catch.stdz[n]) 
		}	
	}
}
...
###########################################
res<-6
name_figure <- "Figure_Group_Catch_Panel_All.png"
