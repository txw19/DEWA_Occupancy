...
for(i in 1:out$BUGSoutput$n.sim){ # simulations
	for(t in 1:T){ #species
		for(n in 1:length(new.slope.stdz)){ # species
			probs[i,t,n] <- plogis(out$BUGSoutput$sims.list$alpha.occ[i,t] + out$BUGSoutput$sims.list$beta6[i,t] * new.slope.stdz[n])
		}	
	}
}
...
spp <- c(12,26,30)
# spp.names <- c('Creek Chub','Shield Darter','White Sucker')
...
###########################################
res<-6
name_figure <- "Figure_Slope_Panel_Subset.png"
png(filename = name_figure, height = 400*res, width = 920*res, res=72*res)

def.par <- par(no.readonly = TRUE) 

size.label = 1
size.text = 1.0
x.label = 'Stream slope'
...
layout.show(nf)
par(mar=c(0.5,0.5,0.5,0.5),oma=c(3,4,0.5,0.5),mai=c(0.1,0.1,0.0,0) )	
...
points(new.slope.stdz, mean.probs[i, ], cex=0.8, pch=16,type='l',lty=1, lwd=2)

# title(main=spp.names[i], line=0.2)

text(-1.5, 0.85, lett[i], cex=1.2)


axis(side=1,cex.axis=1.5, mgp=c(1,0.6,0),tck= -0.01,at=xlab1, labels=xlab2)	

if( i == 1 ){
	axis(side=2,cex.axis=1.5, mgp=c(0,0.6,0),tck= -0.01, las=1)
	} else {
	axis(side=2,cex.axis=1.5, mgp=c(0,0.2,0),tck= -0.01, labels=F)
