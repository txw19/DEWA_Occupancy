...
new.catch <- seq(min(discharge3),max(discharge3),length=50)
mean(loglwd)
...
for(i in 1:out$BUGSoutput$n.sim){ # simulations
  for(t in 1:T){ #species
    for(n in 1:length(new.lwd)){ # species
      probs[i,t,n] <- plogis(out$BUGSoutput$sims.list$alpha.p[i,t] + out$BUGSoutput$sims.list$beta.p3[i,t] * new.catch.stdz[n])
    }	
  }
}
...
###########################################
res<-6
name_figure <- "Figure_Discharge_detect_Panel_Subset.png"
...
size.label = 1
size.text = 1.0
x.label = expression(paste('Discharge (',m^3,'/s)'))
y.label = 'Detection probability'

range(discharge3)
new.cond <- seq(min(discharge3),max(discharge3),length=50)

xlab1 <- c(
  (0-mean(new.cond))/sd(new.cond ),
  (0.2-mean(new.cond))/sd(new.cond),
  (0.4-mean(new.cond))/sd(new.cond), 
  (0.6-mean(new.cond))/sd(new.cond),
  (0.8-mean(new.cond))/sd(new.cond),
  (1.0-mean(new.cond))/sd(new.cond)
)
