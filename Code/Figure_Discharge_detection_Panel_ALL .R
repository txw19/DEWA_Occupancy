# Create new lwd data 
new.lwd1 <-seq(min(discharge3),max(discharge3),length=50)
...
for(i in 1:out$BUGSoutput$n.sim){ # simulations
	for(t in 1:T){ #species
		for(n in 1:length(new.lwd)){ # species
			probs[i,t,n] <- plogis(out$BUGSoutput$sims.list$alpha.p[i,t] + out$BUGSoutput$sims.list$beta.p3[i,t] * new.lwd[n])
		}	
	}
}
...
###########################################
res<-6
name_figure <- "Figure_Discharge_detection_Panel_ALL.png"
