...
for(i in 1:out$BUGSoutput$n.sim){ # simulations
	for(t in 1:T){ #species
		for(n in 1:length(new.lwd)){ # species
			probs[i,t,n] <- plogis(out$BUGSoutput$sims.list$alpha.occ[i,t] + out$BUGSoutput$sims.list$beta7[i,t] * new.lwd[n])
		}	
	}
}
...
spp <- c(3,25)
spp.names <- c('Blacknose Dace','Sea Lamprey')
...
###########################################
res<-6
name_figure <- "Figure_LWD_Panel_Subset.png"
...
plot(new.lwd, mean.probs.sub[i, ], axes=F, ylim=c(min(probsLCI.sub),max(probsUCI.sub) ), ylab='', xlab='', type='n')
...
points(new.lwd, mean.probs.sub[i, ], cex=0.8, pch=16,type='l',lty=1)
