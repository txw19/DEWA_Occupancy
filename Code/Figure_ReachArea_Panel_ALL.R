...
new.area <- seq(min(area),max(area),length=50)

# Standardize new forest variable
new.area.stdz <- (new.area - mean(new.area))/sd(new.area)

T <- length(unique(df4$species))

# container for predicted value
probs <- array(NA,c(out$BUGSoutput$n.sim,T,length(new.area.stdz) ) )
dim(probs)

for(i in 1:out$BUGSoutput$n.sim){ # simulations
	for(t in 1:T){ #species
		for(n in 1:length(new.area.stdz)){ # species
			probs[i,t,n] <- plogis(out$BUGSoutput$sims.list$alpha.p[i,t] + out$BUGSoutput$sims.list$beta.p[i,t] * new.area.stdz[n])
		}	
	}
}

mean.probs <- matrix(NA, nrow=T, ncol=length(new.area.stdz))
for(t in 1:T){
	for(n in 1:length(new.area.stdz)){
...
### CIs
probsLCI <- matrix(NA, nrow=T, ncol=length(new.area.stdz))
for(t in 1:T){
	for(n in 1:length(new.area.stdz)){
...
probsUCI <- matrix(NA, nrow=T, ncol=length(new.area.stdz))
for(t in 1:T){
	for(n in 1:length(new.area.stdz)){
...
###########################################
res<-6
name_figure <- "Figure_ReachArea_Panel_ALL.png"
...
size.label = 1
size.text = 1.0
x.label = 'Reach Area (m^2)'
y.label = 'Occurrence probability'

range(area)

xlab1 <- c(
(100-mean(new.area))/sd(new.area),
(300-mean(new.area))/sd(new.area),
(600-mean(new.area))/sd(new.area), 
(900-mean(new.area))/sd(new.area),
(1200-mean(new.area))/sd(new.area)
)


xlab2 <- xlab1 * sd(new.area) + mean(new.area)
...
plot(new.area.stdz, mean.probs[i, ], axes=F, ylim=c(min(probsLCI),max(probsUCI) ), ylab='', xlab='', type='n')
	
i.for <- order(new.area.stdz )
i.back <- order(new.area.stdz , decreasing = TRUE )

x.polygon <- c(new.area.stdz[i.for] , new.area.stdz[i.back] )
y.polygon <- c(probsLCI[i, ][i.for] , probsUCI[i, ][i.back] )

polygon( x.polygon , y.polygon , col = "gray" , border = NA )

points(new.area.stdz, mean.probs[i, ], cex=0.8, pch=16,type='l',lty=1)
